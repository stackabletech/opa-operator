...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-opa-operator") | .version')
...
version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/opa-operator/Chart.yaml
	yq eval -i '.version = strenv(DOCS_VERSION) | .prerelease = strenv(IS_NIGHTLY) != ""' /dev/stdin < docs/antora.yml

config:
	if [ -d "deploy/config-spec/" ]; then\
		mkdir -p deploy/helm/opa-operator/configs;\
