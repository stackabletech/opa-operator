...
If the derived value doesn't suit your cluster, you can override it as described in <<override-the-traffic-policy>>.
...
To ease the interruption, pin the traffic policy to Cluster as described in <<override-the-traffic-policy>>, and remove the override once the rollout has finished.
