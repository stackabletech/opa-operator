...
====
As of the SDP release 24.07 we document an example rego rule here making an HTTP call.
...
This enables us e.g. to make breaking changes to the HTTP API while still keeping the rego rules API stable.

The documentation will be updated to use the deployed rego rules instead of containing an http-call example.
====
