...
To deploy OPA, you just need to create an OpaCluster resource in Kubernetes and the Operator will create OPA instance. Create a file called `opa.yaml` with the following contents:
...
Now deploy first rule to the Agent. Rules are deployed in ConfigMaps. Create a file `simple-rule.yaml` with the following contents:
...
Now that you have deployed the rule, you can query OPA for it. First, port-forward the service to connect to OPA:
